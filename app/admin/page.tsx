      ...
      <main className="min-h-screen bg-black text-green-400 flex items-center justify-center p-6">
        <section className="bg-black border border-red-500/50 rounded-2xl shadow-[0_0_30px_rgba(239,68,68,0.2)] p-10 max-w-md w-full text-center backdrop-blur-md">
          ...
          <p className="text-gray-400 mb-8">
            Autorisation insuffisante. Seuls les administrateurs peuvent accéder à ce terminal.
          </p>
          <Link
            href="/"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gray-950/50 border border-green-900 rounded-xl p-6 shadow-inner">
              <h3 className="text-xs font-bold text-green-700 uppercase mb-4 tracking-widest">Identité</h3>
              <p className="text-xl font-bold text-green-100 truncate">
                {session?.user?.name || "Admin"}
              </p>
              <p className="text-green-800 font-mono text-sm">{session?.user?.email}</p>
            </div>
            ...
            {winner ? (
              <div className="bg-green-500/5 border-2 border-yellow-500/50 rounded-2xl p-6 relative overflow-hidden group">
                <div className="absolute -right-4 -top-4 text-yellow-500/10 rotate-12 transition-transform group-hover:scale-110">
                   <svg width="120" height="120" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z"/></svg>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-yellow-500 font-mono text-2xl">{winner.score} <span className="text-sm">PTS</span></p>
                </div>
                <div className="mt-6 pt-4 border-t border-yellow-500/20 text-xs text-gray-500 font-mono">
                   SOUCHON_DATE: {new Date(winner.createdAt).toLocaleDateString("fr-FR")}
                </div>
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-green-500/10 border-b border-green-500/30">
                      <th className="px-6 py-4 font-bold text-green-400 uppercase text-xs">Rang</th>
                      <th className="px-6 py-4 font-bold text-green-400 uppercase text-xs">Utilisateur</th>
                      <th className="px-6 py-4 font-bold text-green-400 uppercase text-xs text-right">Score</th>
                      <th className="px-6 py-4 font-bold text-green-400 uppercase text-xs text-right">Date</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-green-900/30">
                    {leaderboard.length > 0 ? (
                      leaderboard.map((entry, index) => (
                        <tr key={`${entry.email}-${entry.createdAt}`} className="hover:bg-green-500/5 transition-colors group">
                          <td className="px-6 py-4">
                            <span className={`font-mono font-bold ${index === 0 ? 'text-yellow-500' : 'text-gray-500'}`}>
                              #{String(index + 1).padStart(2, '0')}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <div className="font-bold text-green-100 group-hover:text-green-400 transition-colors">{entry.name}</div>
                            <div className="text-xs text-gray-600 font-mono truncate max-w-[150px] md:max-w-xs">{entry.email}</div>
                          </td>
                      ...
                      <tr>
                        <td colSpan={4} className="p-12 text-center text-gray-600 font-mono">
                          NO_RECORDS_FOUND
                        </td>
                      </tr>
                    )}
